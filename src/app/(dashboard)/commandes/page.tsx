...
export default async function CommandesPage({ searchParams }: PageProps) {
  ...
  const [commandes, fournisseurs] = await Promise.all([
    listerCommandesTransverse({
      fournisseurId: sp.fournisseur,
      categorie: sp.categorie,
      statutCommande: sp.statutCmd,
      statutLivraison: sp.statutLiv,
      semaine: sp.semaine,
      recherche: sp.q,
    }),
    listerFournisseurs(),
  ...
  const semainesUniques = Array.from(
    new Set(commandes.map((c) => c.semaineLivraisonPrevue).filter(Boolean)),
  ).sort() as string[];
    ...
    <div className="flex flex-col gap-4 p-4">
      <header>
        ...
        <p className="text-xs text-slate-500">
          {commandes.length} commande{commandes.length > 1 ? "s" : ""} · vue
          transverse filtrée
        </p>
      </header>
            ...
            {commandes.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50">
                ...
                <td className="px-3 py-2">
                  {c.projet.client.prenom} {c.projet.client.nom}
                </td>
                <td className="px-3 py-2 text-slate-700">
                  {LIBELLES_CATEGORIE[c.categorie]}
                </td>
                <td className="px-3 py-2 text-slate-700">{c.fournisseur.nom}</td>
                <td className="px-3 py-2">
                  <span
                    className={cn(
