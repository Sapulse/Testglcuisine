...
/** Duplique un projet (client et relations conservés, statuts remis à zéro). */
export async function dupliquerProjet(
  id: string,
): Promise<ActionResult<{ id: string }>> {
  try {
    const source = await prisma.projet.findUnique({
      where: { id },
      include: { commandes: true },
    });
    if (!source) return { ok: false, message: "Projet introuvable" };

    const suffixe = Date.now().toString(36).slice(-4).toUpperCase();
    const nouvelle = await prisma.$transaction((tx) =>
      tx.projet.create({
        data: {
          reference: `${source.reference}-C${suffixe}`,
          clientId: source.clientId,
          vendeurId: source.vendeurId,
          typeProjet: source.typeProjet,
          adresseChantier: source.adresseChantier,
          codePostalChantier: source.codePostalChantier,
          villeChantier: source.villeChantier,
          montantHT: source.montantHT,
          montantTTC: source.montantTTC,
          semainePose: source.semainePose,
          anneePose: source.anneePose,
          estRenovation: source.estRenovation,
          etapes: {
            create: NOMS_ETAPES.map((nom, i) => ({
              numero: i + 1,
              nom,
          ...
          commandes: {
            create: source.commandes.map((c) => ({
              fournisseurId: c.fournisseurId,
              categorie: c.categorie,
              statutCommande: "non_envoye" as const,
              semaineLivraisonPrevue: null,
              statutLivraison: "en_attente" as const,
    ...
    revalidatePath("/projets");
    revalidatePath("/");
    return { ok: true, data: { id: nouvelle.id } };
  } catch (e) {
    return { ok: false, message: e instanceof Error ? e.message : "Erreur inconnue" };
  }
}
